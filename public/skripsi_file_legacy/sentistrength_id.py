...
mydb = mysql.connector.connect(host="localhost", user="root", password="", database="skripsi")

mycursor = mydb.cursor(dictionary=True)

mycursor.execute("SELECT * FROM dataset WHERE jenis='umum' AND status='pakai'")

myresult = mycursor.fetchall()

for x in myresult:
    hasil_senti = senti.main(x['pra'])
    string_sentistrength = str(hasil_senti)

    idnya_data = str(x['id'])
    sql = "UPDATE dataset SET sentistrength = %s WHERE id = %s"
    value = (string_sentistrength, idnya_data)
